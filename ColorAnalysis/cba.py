...
def correction_and_analysis(colorBoard, display=1):
    ...
    rect_point = cbe.find_edge_of_colorboard(
        colorBoard, False)  # 找尋邊緣，並回傳矩形的頂點座標
    print(rect_point)
    try:
        if len(rect_point) != 4:
            ...
            script_path = os.path.join(
                parent_path, "Color-Chart-Measurement-master", "src", "main.py")
            res = subprocess.run(["python", script_path])
            if res.returncode == 0 and os.path.exists(output_file):
                with open(output_file, 'r') as f:
                    points_str = f.read()
                    #print("artificial selection points:", points_str)
                    # 解析座標值
                    # 使用正規表達式匹配座標
                    pattern = r"\((\d+), (\d+)\)"
                    ...
                    print("artificial selection points:", points)
                    rect_point = points
            #print("artificial selection points:", res)
            # 刪除暫存的檔案
    ...
    dc_img = cbe.perspective_correction(
        colorBoard_copy, rect_point, False)  # 對矩形的頂點座標做透視校正
    dc_img = cbe.rotate(dc_img)  # 旋轉
    # 處理並輸出色塊的代表顏色，colorBlockVal[i][j]為色塊顏色
    colorBlockVal = color_analysis(dc_img)
...
def compare_colorboard(a_val, b_val):  # 色差計算
    ...
    # 繪製表格並設定顏色
    table = ax.table(cellText=cell_text, cellLoc='center', bbox=[
                     0, 0, 0.8, 1], cellColours=plt.cm.Greens(delta_e_norm))
    ...
    sm = plt.cm.ScalarMappable(
        cmap=plt.cm.Greens, norm=plt.Normalize(vmin=0, vmax=1))
    sm.set_array([])
...
def color_diff_colorboard(a_val, b_val):
    ...
    for i in range(0, 5):
        row = []
        for j in range(0, 5):
            row.append(get_color_channel_diff_percent(
                a_val[i][j], b_val[i][j]))
        diff.append(row)
